...
lines.push('## 更新の一覧')
lines.push('')
lines.push('| 日付 | バージョン | 項目数 | 追加 | 変更 | 修正 |')
lines.push('| --- | :--: | ---: | ---: | ---: | ---: |')
lines.push(...handRows)           // 手書きぶんは新しいはずなので上に
for (const g of DATA.groups) {
  const k = { 追加: 0, 変更: 0, 修正: 0 }
  for (const t of g.items) k[kindOf(t)]++
  lines.push(`| [${cell(g.date)}](#${anchorOf(g.date)}) | ${g.version ? cell(g.version) : '—'} | ${g.items.length} | ${k.追加 || '—'} | ${k.変更 || '—'} | ${k.修正 || '—'} |`)
}
lines.push('')
...
for (const g of DATA.groups) {
  lines.push(`## ${g.date}`)
  lines.push('')
  lines.push(`${g.version ? `DQMVI ${g.version} ・ ` : ''}${g.items.length}項目`)
  lines.push('')
