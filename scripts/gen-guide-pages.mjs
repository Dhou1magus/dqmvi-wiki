 ...
 * ★載せないと決めた項目（導きの書のid）。
 *
 * 導きの書はゲーム内で読めるものなので基本はそのまま載せるが、
 * **終盤の筋書きに触れるものは載せない**（2026-09-01 よっしー判断）。
 * 「攻略を調べに来ただけの人が、終盤の展開を先に知ってしまう」のを避けるため。
 *
 * q001〜q004 … マスタードラゴンからのメインクエスト一式
 *   （受注のしかた / 魔王の証 / 魔王の間 / 六魔王）。
 *   ゲーム終盤に来る内容で、そのまま筋書きのネタバレになる。
 * h098〜h099 … 施設と暮らし＞ボス（祭壇のボス / 討伐の証）。
 *   魔王ボスの情報は全部出さない方針（2026-09-02 よっしー指示「ボスモンスターの情報全削除」）。
 *
 * ここに足すとどのページからも消える（一覧・検索も含む）。
 * 小分類ごと消したいときは PAGES の exclude を使う。
 */
const SPOILERS = new Set(['q001', 'q002', 'q003', 'q004', 'h098', 'h099'])

/**
 * 本文の中に混ざる終盤の固有名詞を伏せる。項目ごと消すほどではないもの用。
 * 例: ガンビットの決まり(h148)に「ブロックを置けない場所(魔王の間など)」と出てくる。
 *     決まり自体は序盤から役に立つので残し、場所の名前だけ落とす。
 */
const REDACT = [
  [/\(魔王の間など\)/g, ''],
  [/（魔王の間など）/g, '']
]
const redact = (text) => REDACT.reduce((t, [re, to]) => t.replace(re, to), String(text ?? ''))
...
function readGuide(path) {
  return readFileSync(path, 'utf8')
    ...
    .map((l) => {
      ...
      return { id, stage, major, minor, title, body: redact(rest.join('\t') ?? '') }
    })
    .filter((e) => e.major && e.title)
    .filter((e) => !SPOILERS.has(e.id))
}
...
function spellsPage() {
  ...
  const path = join(DOCS, 'spells', 'index.md')
  const extra = extraRows(path, new Set(list.map((s) => plainName(s.title))))
...
function skillsPage() {
  ...
  const path = join(DOCS, 'skills', 'index.md')
  const extra = extraRows(path, new Set(list.map((s) => plainName(s.title))))
...
const PLAY = [
  ...
  { major: 'クエスト', slug: 'quests', lead: '町の人やお店からの依頼の受け方と報酬。' }
]
...
function playPage(page) {
  ...
  lines.push('')
  return { text: finish(lines.join('\n'), join(DOCS, 'play', `${page.slug}.md`)), count, title }
}
...
function playIndex(made) {
  const path = join(DOCS, 'play', 'index.md')
  const extra = extraRows(path, new Set(made.map((m) => plainName(m.title))))
  const lines = []
...
// 呪文と特技は一覧ページの表の1行。名前で引けるように索引へ足す
recordNames('guide', [
