...
function monsterIndex(normals) {
  ...
  lines.push(`description: DQMVIに登場するモンスター${normals.length}体のランク・HP・こうげき・しゅび・経験値・ゴールドの一覧。ゲーム内の図鑑ナンバー順。`)
  ...
  lines.push('番号はゲーム内のものをそのまま使っているので、ところどころ飛びます。')
  lines.push('')
  lines.push('**ランク**は図鑑画面の「ランク」と同じ数字です。ランク3のモンスターは、ランク3の土地（バイオーム名の横に出る数字）に湧きます。')
  lines.push('')
  lines.push('::: tip 探し方')
