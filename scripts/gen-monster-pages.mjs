 ...
 *   boss_ai.tsv        … 魔王ボスの id（除外の判定にだけ使う）
...
//   一覧の行は空のマス、系統の一覧には出さない（gen-drop / gen-item の逆引きも同じ id で外す）。
...
// ── 出現場所の書き方 ──────────────────────────────────────
// 図鑑の「ランク」から引ける言い方だけを書く（どのバイオームに湧くかは載せない）。
function placesCell(x) {
  return x.rank ? `ランク${x.rank}のふつうの土地` : ''
}
...
function monsterPage(m) {
  ...
  if (blank) {
    ...
    // ランクの無い個体は野生の湧きが無いので行ごと出さない
    if (x?.rank) lines.push(`| 出現場所 | ${placesCell(x)} |`)
  }
  lines.push('')
  ...
  const path = join(MON_DIR, `${m.id}.md`)
...
//   図鑑・系統・ドロップ逆引きのどこにも出さない。番号は振り直さない。
