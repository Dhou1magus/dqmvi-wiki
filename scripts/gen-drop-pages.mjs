...
const TAB_JA = new Map([
  ...
  ['FISHING', '釣り'], ['SPECIAL_ITEMS', '特殊'], ['MONUMENTS', 'モニュメント'],
...
function itemPage(it) {
  ...
  lines.push('## 落とすモンスター')
  lines.push('')
  lines.push('| モンスター | 区分 | 系統 | HP | EXP | 出現場所 |')
  lines.push('| --- | :--: | :--: | ---: | ---: | --- |')
...
// 前回作ったが今回は要らなくなったページを消す（MODから消えたアイテムなど）
const want = new Set(['index.md', ...items.map((i) => `${i.slug}.md`)])
let removed = 0
for (const f of readdirSync(OUT_DIR)) {
  if (f.endsWith('.md') && !want.has(f)) { rmSync(join(OUT_DIR, f)); removed++ }
}
